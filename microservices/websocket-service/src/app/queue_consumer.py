...
def start_consumer(socketio, rabbitmq_url, logger):
    ...
    while True:
        try:
            ...
            logger.info("Consumidor do RabbitMQ conectado.")
            ...
            def callback(ch, method, properties, body):
                logger.info(f" [x] Recebido de {method.routing_key}: {body}")
                try:
                    ...
                    logger.error("Não foi possível decodificar a mensagem da fila.")
            ...
            logger.info(' [*] Aguardando por mensagens. Para sair, pressione CTRL+C')
            channel.start_consuming()

        except pika.exceptions.AMQPConnectionError as e:
            logger.error(f"Não foi possível conectar ao RabbitMQ: {e}. Tentando novamente em 5 segundos...")
            time.sleep(5)
        except Exception as e:
            logger.error(f"Ocorreu um erro inesperado: {e}. Reiniciando o consumidor...")
            time.sleep(5)
